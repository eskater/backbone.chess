(function(dom, ws) {
    ...
    application.extendModel('game', 'game-blitz', {
        ...
        position: function(route) {
            if (!this.get('stop') && !this.get('pause')) {
                this.set({
                    start: true,
                    color: this.route.players().color()
                });
            }
        },
        pat: function(event) {
        ...
        stop: function() {
            ...
            _.invoke(this.route.board().models, 'set', {
                active: false
            });
        ...
        render: function() {
            ...
            _.each(this.collection.models, function(player) {
                if (that.player(player.color())[0]) {
                    ...
                    that.$el.append(_.template(jQuery('#template-player').html())({
                        player: player
                    }));
                }
            });
    ...
    application.setView('history', {
        ...
        render: function() {
            this.$el.html(
                _.template(jQuery('#template-history').html())({
                    player: this.model.players().findWhere({
                        current: true
                    }),
        ...
        detail: function(event) {
            ...
            var that = this,
                row = this.history.get({
                    cid: jQuery(event.currentTarget).data('cid')
                });
    ...
    chess.transport(
        function(transport, route) {
            ...
            application.instanceView('players', {
                el: dom,
                collection: players
            });

            route.viewFen = application.instanceView('fen', {
                model: fen
            });
            route.viewGame = application.instanceView('game-%s'.replace(/%s/, type), {
                el: dom,
                model: game
            });
            route.viewBoard = application.instanceView('board', {
                el: dom,
                model: route
            });
            route.viewHistory = application.instanceView('history', {
                el: dom,
                model: route
            });
            ...
            route.listenTo(route.board(), 'change:position', function(figure) {
                ...
                transport.send('position', 'change', {
